...
### 新增
- 評量考核指派管理的隨機快選新增「受評採樣人數」與「同職稱最大數」設定，產生預覽時會依設定限制每位受評者的評核者數量與同 `jobTitle` 評核者上限。
- 隨機快選預覽清單支援移除整位受評者 row；確認儲存時只寫入仍保留在預覽中的非鎖定 pending 指派。

### 變更
- 指派管理對話框加寬並整理隨機快選操作列，讓「受評採樣人數」、「同職稱最大數」與「隨機快選／重新隨機」按鈕在桌面寬度下維持同一水平列。

### 文件
- 定義評量考核「隨機快選」參數化規格：受評採樣人數由介面輸入且預設 10；同職稱最大數以 `jobTitle` 為判斷欄位，可由介面輸入，未輸入時依各受評者同職稱候選人數自動計算。
- 補充隨機快選預覽可移除整位受評者 row 的規格；確認儲存時只寫入仍保留在預覽中的非鎖定指派，且不異動 completed 指派。

### 測試
- 新增隨機快選 options 與預覽移除受評者的測試契約，作為後續服務層與指派管理介面實作的驗收基準。
- 補強隨機快選演算法測試，覆蓋自訂採樣人數、同職稱上限、自動同職稱上限與移除預覽受評者 row 後的儲存行為。
