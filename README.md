...
- 僅在使用者登入後由共用 Layout 顯示，並依 `Asia/Taipei` 時區自動選取當月 Google Sheet（例如 `2026/7_班表`）與今日資料。
- 面板顯示今日日期、早班、中班、兩位 On-call 人員，以及可另開分頁查看的原始班表連結。
- 使用者可在基本資料的生日下方選填 Telegram 使用者名稱；班表姓名符合且帳號格式合法時，Ops Duty 會將姓名顯示為可直接聯繫的 `https://t.me/{username}` 連結。
- 採半透明、緊湊且與主站有區隔的等寬字型設計；可在畫面內自由拖曳，也可用右上角關閉按鈕暫時隱藏至下次重新載入。
- 公開班表透過 Google Visualization callback 讀取，不需在前端固定每月工作表 gid；載入失敗時會顯示錯誤與重新載入操作。
...
- 排程觸發時間接受台北 17:30:00（含）至 17:35:00（不含）的偏移，修正數秒延遲造成的 `INVALID_SCHEDULE_TIME`；報表截止仍固定當日 17:30:00，時間不得在未來，且投遞距觸發時間須未滿 24 小時。
...
- 「補助上限」內所有補助進度條統一使用 8px 高度，與 Training + AI Tool 共用池進度條維持一致的視覺粗度。
- 共用剩餘額度公式為 `max(0, 24,000 − Training 已核准金額 − AI Tool 已核准金額)`。

### 筆電補助分期追蹤

- 管理員可在 `/Subsidy/List` 的「筆電分期未領完」Tab 集中查看已核准、但尚未完成所有預定分期的筆電補助申請。
- 清單依發票金額計算既有筆電補助分期計畫，並即時監聽各申請的 `installments` 子集合；完成最後一期後，該筆申請會自動移出清單。
- 此 Tab 固定呈現全體待領完紀錄，不套用補助類型或日期條件；切換進入時會停用並關閉篩選面板，但保留原篩選值，返回其他 Tab 後可繼續使用。
- 每筆紀錄保留申請歷史與分期記錄入口，方便管理員直接檢視稽核軌跡或登錄下一期款項。
...
- 遠端 PDF 下載後會依附件 metadata 正規化 Blob MIME，再建立預覽 URL，避免 Storage 回應型別缺失或錯誤時將 PDF 二進位內容顯示成文字亂碼。
...
- 事件新增／更新／刪除失敗時，只顯示 `JourneyEventService` 已在地化的繁中訊息；非預期錯誤（Firebase SDK、瀏覽器 API）改用通用提示並記錄到 console，不將 Firestore 規則細節暴露給使用者（GitHub issue #27）。
- 「我的職場屬性報告」在登入狀態失效時顯示明確空狀態，不再讓時間軸區塊直接消失（GitHub issue #27）。
...
1. **管理者建立考核週期並指派評核關係**：設定週期名稱、年份（H1/H2）、截止日，並為每位受評者指派一至多位評核者。管理者也可使用「隨機快選」，一次為所有在職且非管理員的使用者產生可編輯預覽；系統會排除自評，依介面設定的受評採樣人數與同職稱最大數產生建議，盡量平均評核負載並優先安排同職稱評核者。管理者可在預覽中調整評核者或移除整位受評者，確認後才寫入正式指派，且不會異動已完成指派。
