...
2. **評核者填寫匿名考評表（10 題評分 + 整體評價）**：系統開放評核者填寫 10 道職場行為題目（每題 1–10 分），各題均附行為觀察 hint 說明，協助評核者理解評估重點；並於表末填寫整體評語（20–500 字）。分數 ≥9 或 ≤3 時，回饋說明欄自動轉為必填。任務提交後會移至「已填寫」，且在截止日前可再次編輯已提交內容。
3. **管理者結束並發布（觸發 Z-score 校正計算）**：管理者確認後執行「結束並發布」，系統對每位評核者進行 per-rater Z-score 校正（TARGET_MEAN=5.5, SD=1.5, clamp 1–10），彙整六大屬性分數，決定職業原型，並將 snapshot 狀態更新為 `final`。整體評語跑馬燈內容於此階段由該受評者的所有考評表重新彙整，與「有效評核者人數」同源，確保評語數與評核人數一致。
...
| **評核者**（Evaluator） | 查看分配給自己的考評任務、填寫並提交匿名考評表、截止日前可填寫與編輯已填寫任務 |
